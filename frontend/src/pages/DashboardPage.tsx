...
import { 
  ...
  Terminal, Cpu, Network, Plus, Download,
  RefreshCcw, ChevronRight, Bell, Server
} from 'lucide-react';
...
export const DashboardPage = () => {
  ...
  const [logs, setLogs] = useState([
    { id: 1, time: 'LIVE', msg: 'System integrity monitor active', type: 'secure' },
    { id: 2, time: 'LOG', msg: 'Database connection established', type: 'info' },
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSystemLoad(prev => Math.floor(Math.max(30, Math.min(60, prev + (Math.random() * 10 - 5)))));
    }, 2000);
  ...
  const handleRefresh = () => {
    ...
    fetchMetrics();
    setLogs(prev => [
      { id: Date.now(), time: new Date().toLocaleTimeString([], {hour12:false}), msg: 'Manual synchronization complete', type: 'info' },
      ...prev.slice(0, 4)
    ]);
  };
  ...
  const safeStats = {
    ...
    severity: metrics?.severityDistribution || { Low: 0, Medium: 0, High: 0, Critical: 0 },
    riskTrends: metrics?.riskTrends || [] // Default to empty array if missing
  };

  // Calculate Risk Score Safely
  const currentRiskScore = safeStats.riskTrends.length > 0 
    ? safeStats.riskTrends[safeStats.riskTrends.length - 1].riskScore 
    : 0;

  // Determine Status
  const systemStatus = currentRiskScore > 70 ? 'critical' : currentRiskScore > 40 ? 'warning' : 'secure';
    ...
    return (
      <DashboardLayout>
        <div className="h-[80vh] flex flex-col items-center justify-center">
           ...
           <p className="text-cyber-blue font-mono text-sm tracking-widest animate-pulse">ESTABLISHING SECURE CONNECTION...</p>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-white">COMMAND_CENTER</h1>
            <p className="text-gray-500 text-xs font-bold uppercase tracking-[0.3em] mt-1 flex items-center">
               ...
               VULNEXA_CORE v4.0.0 // USER: {user?.name?.toUpperCase() || 'ADMIN'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'CPU_USAGE', value: `${systemLoad}%`, icon: Cpu, color: 'text-cyber-blue' },
            { label: 'NETWORK_LATENCY', value: '24ms', icon: Network, color: 'text-purple-500' },
            { label: 'TOTAL_SCANS', value: safeStats.totalScans, icon: Terminal, color: 'text-white' },
            { label: 'VULN_DETECTED', value: safeStats.totalVulnerabilities, icon: Zap, color: safeStats.totalVulnerabilities > 0 ? 'text-red-500' : 'text-emerald-500' },
          ].map((stat, i) => (
            <GlassCard key={i} className="flex items-center justify-between p-5 border-white/5 hover:border-white/20 transition-colors cursor-default group">
               <div>
                 <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{stat.label}</p>
                 <p className={`text-2xl font-black mt-1 ${stat.color}`}>{stat.value}</p>
               </div>
               <stat.icon className="text-gray-700 group-hover:text-white transition-colors" size={24} />
            </GlassCard>
          ))}
        </div>
            ...
            <GlassCard className="h-full min-h-[400px] border-t-2 border-t-cyber-blue/30 overflow-hidden flex flex-col">
               <div className="flex justify-between items-center mb-10">
                  <div className="flex items-center space-x-3">
                    ...
                    <h3 className="text-lg font-bold text-white">Threat Monitoring Stream</h3>
                  </div>
               ...
               <div className="flex-1 px-4 space-y-6">
                  {[
                    { label: 'CRITICAL', val: safeStats.severity.Critical, color: 'bg-red-600' },
                    { label: 'HIGH', val: safeStats.severity.High, color: 'bg-orange-500' },
                    { label: 'MEDIUM', val: safeStats.severity.Medium, color: 'bg-yellow-500' },
                    { label: 'LOW', val: safeStats.severity.Low, color: 'bg-emerald-500' },
                  ].map((bar, i) => (
                    <div key={i}>
                      <div className="flex justify-between text-[10px] font-bold uppercase mb-2">
                        <span className="text-gray-400">{bar.label}_VULNERABILITIES</span>
                        <span className="text-white">{bar.val} Found</span>
                      </div>
                      <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                         <motion.div 
                           initial={{ width: 0 }}
                           animate={{ width: `${Math.min(100, (bar.val / (safeStats.totalVulnerabilities || 1)) * 100)}%` }}
               ...
               <div className="mt-8 pt-6 border-t border-white/5 grid grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-[10px] text-gray-500 uppercase">Avg Risk</p>
                    <p className="text-xl font-bold text-white">{currentRiskScore}/100</p>
                  </div>
            ...
            <GlassCard className="h-full flex flex-col">
               <div className="flex items-center justify-between mb-6">
                  <h3 className="text-sm font-black uppercase tracking-widest flex items-center text-white">
                    <Bell size={16} className="mr-2 text-cyber-purple" /> Live_Feed
                  </h3>
                  <span className="text-[10px] font-mono text-gray-600 animate-pulse">● REC</span>
               </div>
               ...
               >
                 Full Audit Log <ChevronRight size={14} className="ml-1 group-hover:translate-x-1 transition-transform" />
               </button>
            </GlassCard>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center space-x-4 hover:bg-white/[0.08] transition-all cursor-pointer">
              <div className="p-3 rounded-lg bg-cyber-blue/10 text-cyber-blue"><Server size={20} /></div>
              <div>
                <p className="font-bold text-sm text-white">Server Health</p>
                <p className="text-[10px] text-gray-500 uppercase">Diagnostic Tools</p>
              </div>
           </div>
           <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center space-x-4 hover:bg-white/[0.08] transition-all cursor-pointer">
              <div className="p-3 rounded-lg bg-purple-500/10 text-purple-500"><Download size={20} /></div>
              <div>
                <p className="font-bold text-sm text-white">Export Data</p>
                <p className="text-[10px] text-gray-500 uppercase">PDF / JSON / CSV</p>
              </div>
           </div>
           <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center space-x-4 hover:bg-white/[0.08] transition-all cursor-pointer">
              <div className="p-3 rounded-lg bg-emerald-500/10 text-emerald-500"><ShieldCheck size={20} /></div>
              <div>
                <p className="font-bold text-sm text-white">Compliance</p>
                <p className="text-[10px] text-gray-500 uppercase">ISO 27001 Status</p>
              </div>
           </div>
        </div>
